...
def generate_plot(monthly_reviews, yearly_reviews, total_reviews_by_movie):
    ...
    for reviews_data, suffix in [(monthly_reviews, " (Monthly)"), (yearly_reviews, " (Yearly)")]:
        for movie in reviews_data['Movie'].unique():
            ...
            trace = go.Scatter(x=movie_data[time_period], y=movie_data['TotalReviews'], mode='lines+markers',
                               name=movie_label, hoverinfo='text')
            fig.add_trace(trace)
        ...
        updatemenus=[
            dict(
                buttons=[
                    dict(label="Yearly",
                         method="update",
                         args=[{"visible": [True if " (Yearly)" in t.name else False for t in fig.data]},
                               {"title": "Total reviews per year"}]),
                    dict(label="Monthly",
                         method="update",
                         args=[{"visible": [True if " (Monthly)" in t.name else False for t in fig.data]},
    ...
    # 导出为HTML
    plot(fig, filename='movie_reviews_over_time.html', auto_open=False, include_plotlyjs='cdn')

if __name__ == "__main__":
    filepath = 'B-JSONData.json'  # 确保这里的路径与您的文件路径一致
